<template>
    <el-card class="w-full h-fit">
        <template #header>
            <div class=" w-fit center-x text-2xl font-semibold">{{ providerName }}提供的套餐</div>
        </template>
        <el-table ref="tableRef" :data="tableData" style="width: 100%; height: fit-content" size="large" row-key="date"
            ...
            <el-table-column fixed="right" label="选择" align="center">
                <template #default="scope">
                    <input type="checkbox" class="w-8 h-8" v-model.value="checkedArr[scope.$index]" @change="handleChange(scope.$index)" />
                </template>
            </el-table-column>
        </el-table>
        <el-pagination background layout="prev, pager, next" :total="total" class="center-x w-fit mt-4"
